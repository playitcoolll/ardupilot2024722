...
float AP_PitchController::_get_rate_out(float desired_rate, float scaler, bool disable_integrator, float aspeed,bool ground_mode)
{
    ...
    bool underspeed = aspeed <= float(9);
    ...
    constrain_float(boost_aoa , 0.5, 6);
    ...
    //姿态反馈pid
    // float boost_pitch = 0.5 * abs(_ahrs.pitch_sensor * 0.01 - 0) + 0.5;
    // constrain_float(boost_pitch , 0.5,10 );
    float boost_pitch = 0.5;
   

    // rate_pid.update_all(desired_rate,degrees(rate_y) , dt, limit_I,boost_pitch);
    ...
    // if ( is_positive(dt)) {//!is_zero(ki_pitch) &&
    //     // Ensure that integrator can only be reduced if the output is saturated
    //     if (!limit_I || ((is_positive(integral) && is_negative(error_my)) || (is_negative(integral) && is_positive(error_my)))) {
    //         integral += ((float)error_my * ki_pitch) * dt;
    //         integral = constrain_float(integral, -200, 200);
    //     }
    // } else {
    //     integral = 0.0f;
    // }
...
float AP_PitchController::get_servo_out(int32_t angle_err, float scaler, bool disable_integrator, bool ground_mode)
{
    ...
    float desired_rate = angle_err_deg * 40;

    // 限制需要的速率(degrees/sec)
    // float limit_pitch = 200;
    // if ( desired_rate < -limit_pitch) {
    //     desired_rate = -limit_pitch;
    // } else if ( desired_rate > limit_pitch) {
    //     desired_rate = limit_pitch;
    // }
